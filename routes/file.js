...
file_route.post("/upload", upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        const insertPermissions = async (emailList, permissionType) => {
            for (const userEmail of emailList) {
                await pool.request()
                    ...
                    VALUES (@user_email, @filename, 'YES');
                `);
            }
        };

        // Check and insert permissions for can_edit, can_read, can_download
        if (can_edit && Array.isArray(can_edit) && can_edit.length > 0) {
            await insertPermissions(can_edit, 'can_edit');
        }

        if (can_read && Array.isArray(can_read) && can_read.length > 0) {
            await insertPermissions(can_read, 'can_read');
        }

        if (can_download && Array.isArray(can_download) && can_download.length > 0) {
            await insertPermissions(can_download, 'can_download');
        }
...
file_route.get('/files', async (req, res) => {
    const client = new ftp.Client();
...
file_route.get('/files/:filename', async (req, res) => {
